...
class MaiGeTranslate {
    ...
    ensureStyles() {
        ...
        const css = `
            ...
            .dct-cacheControl input[type="range"] {
            ...
            .dct-cacheControl input[type="range"]::-webkit-slider-thumb {
                -webkit-appearance: none;
                width: 14px;
                height: 14px;
                border-radius: 50%;
            ...
            .dct-cacheRow input[type="number"] {
                width: 70px;
            }
            .dct-cacheRow button {
                margin-left: auto;
            }
            .dct-cacheControl input[type="range"]::-moz-range-thumb {
                width: 14px;
                height: 14px;
                border-radius: 50%;
                background: #2563eb;
    ...
    injectTranslateButton() {
        ...
        input.type = "text";
        input.placeholder = `输入文本，回车翻译并复制为 ${this.settings.inputTargetLanguage}`;
    ...
    updateTranslateInputPlaceholder() {
        if (this.translateInput) {
            this.translateInput.placeholder = `输入文本，回车翻译并复制为 ${this.settings.inputTargetLanguage}`;
        }
    }
    ...
    setTranslateButtonState(state) {
        ...
        if (this.translateInput) {
            ...
            this.translateInput.placeholder = state === "loading"
                ? "翻译中…"
                : `输入文本，回车翻译并复制为 ${this.settings.inputTargetLanguage}`;
        }
    }
    ...
    buildGeneralSettings() {
        ...
        wrapper.appendChild(this.createCacheControlField());
    ...
    createCacheControlField() {
        return this.createField("译文缓存上限 (MB)", () => {
            const wrapper = document.createElement("div");
            wrapper.className = "dct-cacheControl";

            const controls = document.createElement("div");
            controls.className = "dct-cacheRow";
            ...
            range.max = "50";
            range.step = "1";
            range.value = String(Math.round(clampCacheLimit(this.settings.translationCacheLimitMB)));
            ...
            sizeLabel.textContent = this.formatCacheSize(this.translationCacheSizeBytes);

            const clearBtn = document.createElement("button");
            clearBtn.className = "dct-button secondary";
            clearBtn.textContent = "清空缓存";

            const apply = (value, commit) => {
                const mb = clampCacheLimit(value);
                const str = String(mb);
                if (range.value !== str) {
                    range.value = str;
                }
                ...
                this.settings.translationCacheLimitMB = mb;
                if (commit) {
                    this.saveSettings({ skipRuntimeReset: true });
            ...
            number.addEventListener("input", () => apply(number.value, false));
            ...
            controls.append(range, number, sizeLabel, clearBtn);
    ...
    enforceCacheLimit() {
        if (this.translationCacheSizeBytes > this.getCacheLimitBytes()) {
            this.clearTranslationCache();
        }
    }
    ...
    updateCacheSizeLabel() {
        const label = document.querySelector('.dct-cacheControl [data-role="cache-size"]');
        if (label) {
            label.textContent = this.formatCacheSize(this.translationCacheSizeBytes);
        }
    }
